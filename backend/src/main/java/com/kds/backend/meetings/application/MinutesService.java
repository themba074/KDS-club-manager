...
public class MinutesService {
    ...
    private final MeetingRepository meetings; private final MeetingParticipationRepository participation; private final ClubService clubs; private final FileStorageService storage; private final MemberService members; private final DomainEventPublisher events; private final Clock clock;
    public MinutesService(MeetingRepository meetings,MeetingParticipationRepository participation,ClubService clubs,FileStorageService storage,MemberService members,DomainEventPublisher events,Clock clock){this.meetings=meetings;this.participation=participation;this.clubs=clubs;this.storage=storage;this.members=members;this.events=events;this.clock=clock;}
    ...
    @Transactional public MinutesView publish(UUID actor,UUID meetingId,long version){require(actor,Permission.MEETINGS_WRITE);var meeting=requirePastMeeting(meetingId);var minutes=requireMinutes(meetingId);requireVersion(minutes,version);if(minutes.getBody()==null&&minutes.getAttachmentKey()==null)throw bad("Add minutes before publishing.");minutes.publish(clock.instant());participation.flush();try{events.publish(new MinutesPublished(TenantContext.requireClubId(),meetingId,meeting.getTitle(),members.activeMeetingAudience()));}catch(RuntimeException failure){LOGGER.warn("Minutes published but notification publication failed meetingId={}",meetingId,failure);}return view(minutes);}
    ...
    private com.kds.backend.meetings.domain.MeetingEntity requirePastMeeting(UUID id){var meeting=requireMeeting(id);if(meeting.getStartsAt().isAfter(clock.instant()))throw new ResponseStatusException(HttpStatus.CONFLICT,"Minutes can be captured after the meeting starts.");return meeting;}
