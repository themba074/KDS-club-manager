...
public class ContributionReportController {
    ...
    public ResponseEntity<byte[]> export(@AuthenticationPrincipal Jwt jwt,
        ...
        @RequestParam ContributionReportFormat format)throws IOException{
        ...
        var output=new ByteArrayOutputStream();exporter.write(report,output);
        return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION,ContentDisposition.attachment().filename(filename).build().toString())
            .contentType(MediaType.parseMediaType(exporter.mediaType())).body(output.toByteArray());
    }
}
