...
public class ContributionReportService {
    ...
    public ContributionReport summary(UUID actor,LocalDate from,LocalDate to){
        ...
        BigDecimal totalExpected=money(rows.stream().map(MemberContributionSummary::expected).reduce(BigDecimal.ZERO,BigDecimal::add));
        BigDecimal totalCollected=money(rows.stream().map(MemberContributionSummary::collected).reduce(BigDecimal.ZERO,BigDecimal::add));
        BigDecimal totalOutstanding=money(rows.stream().map(MemberContributionSummary::outstanding).reduce(BigDecimal.ZERO,BigDecimal::add));
    ...
    private static BigDecimal money(BigDecimal amount){return amount.setScale(2,RoundingMode.UNNECESSARY);}
    ...
    private static final class MutableMember {
        ...
        private MemberContributionSummary view(){return new MemberContributionSummary(id,name,email,money(expected),money(collected),money(expected.subtract(collected).max(BigDecimal.ZERO)),currency);}
    }
}
