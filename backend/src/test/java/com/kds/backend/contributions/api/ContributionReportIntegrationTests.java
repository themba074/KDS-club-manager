...
class ContributionReportIntegrationTests {
    ...
    @Test void summaryAndExportsShareTenantScopedFigures()throws Exception{
        var owner=account();var club=clubs.create(owner.userId(),"Reporting Club");var ownerSession=auth.selectClub(owner.userId(),owner.refreshToken(),club.id());var member=account();UUID memberId=addMember(member,club.id());LocalDate today=LocalDate.now(Clock.systemUTC());
        ...
        mvc.perform(report(ownerSession,"export",today).param("format","CSV")).andExpect(status().isOk()).andExpect(header().string("Content-Disposition",containsString("contributions-"))).andExpect(content().contentTypeCompatibleWith("text/csv")).andExpect(content().string(containsString("100.10,40.05,60.05,ZAR")));
        MvcResult pdfResult=mvc.perform(report(ownerSession,"export",today).param("format","PDF")).andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_PDF)).andReturn();assertEquals("%PDF",new String(pdfResult.getResponse().getContentAsByteArray(),0,4,StandardCharsets.US_ASCII));
    }
    @Test void reportPermissionAndTenantPredicatesPreventDisclosure()throws Exception{
        var owner=account();var club=clubs.create(owner.userId(),"Private Club");var session=auth.selectClub(owner.userId(),owner.refreshToken(),club.id());var member=account();UUID memberId=addMember(member,club.id());LocalDate today=LocalDate.now(Clock.systemUTC());createSchedule(session,memberId,today);
