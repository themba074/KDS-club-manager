...
class DocumentIntegrationTests {
    ...
    @Test void uploadUpdateAndReplacementKeepEveryDownloadableVersion()throws Exception{Fixture fixture=fixture();JsonNode created=create(fixture.session(),"Rules","Policy",List.of("MEMBER"),"rules.txt","first",201);UUID id=uuid(created,"id"),first=uuid(created.get("versions").get(0),"id");
        ...
        assertEquals(1,jdbc.queryForObject("select count(*) from audit_log where club_id=? and action='DOCUMENT_UPLOADED' and entity_id=?",Integer.class,fixture.club(),id));
        assertEquals(1,jdbc.queryForObject("select count(*) from audit_log where club_id=? and action='DOCUMENT_VERSION_UPLOADED' and entity_id=?",Integer.class,fixture.club(),second));
        assertEquals(1,jdbc.queryForObject("select count(*) from audit_log where club_id=? and action='DOCUMENT_METADATA_UPDATED' and entity_id=?",Integer.class,fixture.club(),id));}
