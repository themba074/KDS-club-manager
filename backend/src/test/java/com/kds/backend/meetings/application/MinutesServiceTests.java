...
class MinutesServiceTests {
    private final MeetingRepository meetings=mock(MeetingRepository.class);private final MeetingParticipationRepository participation=mock(MeetingParticipationRepository.class);private final ClubService clubs=mock(ClubService.class);private final FileStorageService storage=mock(FileStorageService.class);private final MemberService members=mock(MemberService.class);private final DomainEventPublisher events=mock(DomainEventPublisher.class);private final Clock clock=Clock.fixed(Instant.parse("2026-09-04T10:00:00Z"),ZoneOffset.UTC);private final MinutesService service=new MinutesService(meetings,participation,clubs,storage,members,events,clock);private final UUID club=UUID.randomUUID(),actor=UUID.randomUUID(),meetingId=UUID.randomUUID();
    ...
    @Test void notificationFailureDoesNotFailMinutesPublication(){var minutes=mock(MeetingMinutesEntity.class);when(minutes.getVersion()).thenReturn(0L);when(minutes.getBody()).thenReturn("Approved minutes");when(participation.minutes(meetingId)).thenReturn(Optional.of(minutes));doThrow(new IllegalStateException("offline")).when(events).publish(any());assertDoesNotThrow(()->service.publish(actor,meetingId,0));verify(minutes).publish(clock.instant());verify(participation).flush();}
}
