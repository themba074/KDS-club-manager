...
- `club_documents` owns tenant-scoped title/category metadata and an optimistic
  version. `document_visibility_roles` stores a validated allowlist drawn from
  ClubTypeConfig, while `document_versions` appends immutable file metadata and
  storage keys. Composite foreign keys prevent cross-club relationships.
- `DOCUMENTS_MANAGE` callers can list and manage every document in their club.
  `DOCUMENTS_READ` callers receive only documents allowed for their current
  role. Download authorization repeats that check for the requested document
  and version, so guessing an ID cannot bypass visibility.
- A replacement creates a new version and storage object; it never overwrites
  previous bytes. Writes lock the club and recheck permission. Uploads are
  restricted to supported document/image types and 5 MB.
- Local storage remains the default. The Supabase adapter targets a private
  bucket, disables overwrite, and creates five-minute signed download URLs
  only after the API authorization check. Credentials come from environment
  variables.
...
- **Storage abstraction interface** (`FileStorageService`); local development
  uses a tenant-namespaced filesystem adapter, while production can select the
  Supabase private-bucket adapter through configuration.
