...
describe("AppRouter", () => {
  ...
  beforeEach(() => {
    ...
    useAuthStore.setState({
      ...
      activeClub: {
        id: "club-1",
        name: "Ubuntu Investment Club",
        clubType: "INVESTMENT_CLUB",
        administrator: true,
        permissions: ["MEMBERS_READ", "CONTRIBUTIONS_READ"],
      },
      switchingClub: false,
    });
  });
  ...
  it("renders the application shell and dashboard route", () => {
    ...
    expect(
      screen.queryByRole("link", { name: "Open voting" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Data will appear when setup is complete."),
    ).not.toBeInTheDocument();
  });
