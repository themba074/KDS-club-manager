  ...
  return (
    <li className="space-y-3 rounded-xl border bg-card p-4">
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <Button
          type="button"
          disabled={assignment.isPending || roleCode === member.roleCode}
          onClick={() =>
            confirm(
              "Change member role",
              `Change ${member.email} to ${roles.find((role) => role.code === roleCode)?.name}? Their club permissions will change immediately.`,
              () => assignment.mutate({ id: member.id, roleCode }),
            )
          }
        >
          {assignment.isPending ? "Savingâ€¦" : "Save role"}
        </Button>
      </div>
      {assignment.error && (
...
export function RoleManagement() {
  ...
  if (roles.isPending) return <p role="status">Loading rolesâ€¦</p>;
  if (roles.error)
    return (
      <div role="alert">
    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="mt-2 text-muted-foreground">
          Roles group the actions a person can perform in this club.
        </p>
      </header>
      <div className="grid gap-4 md:grid-cols-2">
        {roles.data?.map((role) => (
          <article key={role.code} className="rounded-xl border bg-card p-4">
      ...
      {canManage && (
        <section className="space-y-4">
          ...
          <p className="text-sm text-muted-foreground">
            Each member has one role. Keep at least one administrator. Invite
            new members from the Members page.
          </p>
          {members.isPending && <p role="status">Loading membersâ€¦</p>}
          {members.error && (
            <div role="alert">
              <p>{errorMessage(members.error)}</p>
              <Button onClick={() => void members.refetch()}>Retry</Button>
            </div>
          )}
