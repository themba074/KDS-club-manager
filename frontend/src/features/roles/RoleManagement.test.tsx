...
const { get, put, refresh } = vi.hoisted(() => ({
  get: vi.fn(),
  put: vi.fn(),
  refresh: vi.fn(),
}));
vi.mock("@/features/auth/auth-api", () => ({
  ...
  errorMessage: () => "Assignment denied",
}));
const roles = [
  {
    code: "ADMINISTRATOR",
    name: "Administrator",
    permissions: ["ROLES_READ", "ROLES_MANAGE"],
  },
...
beforeEach(() => {
  ...
  useAuthStore
    ...
    .setSession(
      "token",
      { id: "user", email: "owner@example.com" },
      {
        id: "club",
        name: "Club",
...
function page() {
  render(
    ...
    >
      <PermissionGate permission="ROLES_READ">
        <RoleManagement />
      </PermissionGate>
    </QueryClientProvider>,
  );
...
it("submits a selected role and refreshes session permissions", async () => {
  ...
  await waitFor(() =>
    expect(put).toHaveBeenCalledWith("/role-members/membership", {
      roleCode: "MEMBER",
    }),
  );
...
it("shows read-only roles without requesting the member list", async () => {
  useAuthStore.setState((state) => ({
    activeClub: { ...state.activeClub!, permissions: ["ROLES_READ"] },
  }));
  page();
  expect(
...
it("does not trust the administrator display flag for access", () => {
  useAuthStore.setState((state) => ({
    activeClub: { ...state.activeClub!, permissions: [] },
  }));
  page();
