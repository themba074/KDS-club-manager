...
it("records payments and sends targeted reminders",async()=>{renderWithQuery(<RecordPayment/>);expect(await screen.findByRole("option",{name:/Member One/})).toBeInTheDocument();fireEvent.click(screen.getByRole("button",{name:"Mark payment received"}));await waitFor(()=>expect(post).toHaveBeenCalledWith("/contribution-payments",expect.any(FormData)));fireEvent.click(screen.getByRole("button",{name:"Send reminder"}));await waitFor(()=>expect(post).toHaveBeenCalledWith("/contribution-payments/reminders",expect.objectContaining({membershipId:"member-1"})))})
