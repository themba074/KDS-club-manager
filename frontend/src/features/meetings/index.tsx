...
function NextMeetingOverview({ meeting }: { meeting: Meeting }) {
  const rsvp = useRsvp(meeting.id)
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.45fr_1fr]">
      ...
      <Card>
        ...
        <CardContent>
          {rsvp.isPending ? <p className="text-sm text-muted-foreground">Loading RSVP responses…</p> : counts ? <div className="grid grid-cols-3 gap-2 text-center text-sm"><div className="rounded-lg bg-primary/10 p-2"><strong className="block text-base text-primary">{counts.yes}</strong><span className="text-muted-foreground">Going</span></div><div className="rounded-lg bg-amber-500/10 p-2"><strong className="block text-base text-amber-700">{counts.maybe}</strong><span className="text-muted-foreground">Maybe</span></div><div className="rounded-lg bg-destructive/10 p-2"><strong className="block text-base text-destructive">{counts.no}</strong><span className="text-muted-foreground">Declined</span></div></div> : <p className="text-sm text-muted-foreground">No RSVP responses yet.</p>}
        </CardContent>
      </Card>
    </div>
  )
...
export function MeetingsPage() {
  ...
  return <section className="space-y-7"><header><p className="text-sm font-medium text-primary">Club governance</p><h1 className="mt-1 font-heading text-3xl font-semibold tracking-[-0.035em]">Meetings</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Schedule club meetings, keep agendas in a clear order, and track participation.</p></header>
    {upcoming.data?.[0] && <NextMeetingOverview meeting={upcoming.data[0]} />}
