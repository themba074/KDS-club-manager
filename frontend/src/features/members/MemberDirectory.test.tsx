    ...
    data: [
      {
        ...
        roleCode: "MEMBER",
        status: "ACTIVE",
        joinedOrInvitedAt: "2026-08-28T00:00:00Z",
      },
    ],
  });
...
it("lists members and submits a normalized invitation", async () => {
  ...
  fireEvent.change(screen.getByLabelText("Email"), {
    target: { value: " sipho@example.test " },
  });
  fireEvent.click(screen.getByRole("button", { name: "Send invitation" }));
  await waitFor(() =>
    expect(post).toHaveBeenCalledWith("/member-invitations", {
...
it("keeps invitation controls hidden without write permission", async () => {
  useAuthStore.setState((state) => ({
    activeClub: { ...state.activeClub!, permissions: ["MEMBERS_READ"] },
  }));
  page();
...
it("changes an active membership status", async () => {
  ...
  fireEvent.change(
    screen.getByLabelText("Change status for member@example.test"),
    { target: { value: "SUSPENDED" } },
  );
  expect(patch).not.toHaveBeenCalled();
  fireEvent.click(screen.getByRole("button", { name: "Confirm" }));
  await waitFor(() =>
    expect(patch).toHaveBeenCalledWith("/members/member/status", {
      status: "SUSPENDED",
    }),
  );
});
...
it("inspects previews and confirms a bulk import", async () => {
  ...
  expect(
    await screen.findByText(
      "Found 1 data row. Match the required fields below.",
    ),
  ).toBeInTheDocument();
  ...
  expect(post).toHaveBeenCalledWith(
    "/member-imports/confirm",
    expect.any(FormData),
  );
});

it("leaves membership access unchanged when the review is cancelled", async () => {
  page();
  await screen.findByText("Thandi Ndlovu");
  fireEvent.change(
    screen.getByLabelText("Change status for member@example.test"),
  ...
  expect(screen.getByRole("dialog")).toHaveTextContent("member@example.test");
  expect(screen.getByRole("dialog")).toHaveTextContent("cannot be reactivated");
