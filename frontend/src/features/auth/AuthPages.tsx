...
export function CredentialsPage({ mode }: { mode: "login" | "register" }) {
  ...
  return <Frame title={mode === "login" ? "Welcome back" : "Create your account"}>
    <form className="space-y-4" onSubmit={handleSubmit((values) => mutation.mutate(values, { onSuccess: () => navigate("/") }))}>
      ...
      <Button className="w-full" disabled={mutation.isPending}>{mutation.isPending ? "Please wait…" : mode === "login" ? "Log in" : "Register"}</Button>
    </form>
...
export function ResetPasswordPage() {
  ...
  return <Frame title="Choose a new password"><form className="space-y-4" onSubmit={handleSubmit(({ password }) => mutation.mutate({ token, newPassword: password }, { onSuccess: () => navigate("/login") }))}><label className="block text-sm font-medium">New password<Input type="password" {...register("password", { required: true, minLength: 12 })} /></label>{mutation.error && <p role="alert" className="text-sm text-destructive">{errorMessage(mutation.error)}</p>}<Button className="w-full">Reset password</Button></form></Frame>
}
